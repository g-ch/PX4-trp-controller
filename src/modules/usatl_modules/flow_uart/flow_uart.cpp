...
extern "C" __EXPORT int flow_uart_main(int argc, char *argv[]);
...
//extern orb_advert_t mavlink_log_pub_upflow;
orb_advert_t mavlink_log_pub_upflow = NULL;
...
mavlink_log_info(&mavlink_log_pub_upflow,"[inav] upixels_flow_main on init");
// mavlink_log_critical(mavlink_log_pub_upflow, "test>>>>>>> %8.4f",9.9898);
...
int flow_uart_thread_main(int argc, char *argv[])
{
    mavlink_log_info(&mavlink_log_pub_upflow,"upixels_flow run ");
    ...
    // long checksum = 0; 
    // uint8_t valid = 0;
    int uart_read = uart_init("/dev/ttyS0"); // ttyS3 for fmuv5 ttyS6 for fmuv2

    if(uart_read == 0)
    {
         mavlink_log_critical(&mavlink_log_pub_upflow,"[YCM]flow uart init is failed\n");
         return -1;
    }
    if(set_uart_baudrate(uart_read,19200)==0){
        // mavlink_log_critical(&mavlink_log_pub_upflow,"[YCM]set_uart_baudrate is failed\n");
        return -1;
    }
    mavlink_log_info(&mavlink_log_pub_upflow,"[YCM]flow uart init is successful\n");
